...
Weapon::Weapon(ItemStack* item, PlayerEx* playerex) {
  // Ensure the weapon system initialized
  ...
  if (!nbt->contains("tag") ||
    ...
    this->identifier_ = item->getTypeName();
    this->weapon_exp_ = 0;
    this->ascension_phase_ = 0;
    this->refinement_ = 1;

    // 1-Star and 2-Star weapons do not have refinement
    if (this->GetRarity() <= 2) {
      this->refinement_ = 0;
    }
    ...
    // Refresh the inventory next tick to display properly
    // If refresh immediately, it does not work
    Schedule::nextTick(
        [playerex]() { playerex->GetPlayer()->refreshInventory(); });
  }

  auto data = nbt->getCompound("tag")->getCompound("genshicraft");

  this->identifier_ = nbt->getString("Name");
...
int Weapon::GetAscensionPhase() { return this->ascension_phase_; }
...
int Weapon::GetLevel() {
  ...
  if (rarity == 1) {
    for (int i = 1; i <= 70; ++i) {
      if (Weapon::k1StarLevelWeaponEXPList[i] <= this->weapon_exp_) {
        level = i;
      } else {
        break;
      }
  ...
  if (rarity == 2) {
    for (int i = 1; i <= 70; ++i) {
      if (Weapon::k2StarLevelWeaponEXPList[i] <= this->weapon_exp_) {
        level = i;
      } else {
        break;
      }
  ...
  if (rarity == 3) {
    for (int i = 1; i <= 90; ++i) {
      if (Weapon::k3StarLevelWeaponEXPList[i] <= this->weapon_exp_) {
        level = i;
      } else {
        break;
      }
  ...
  if (rarity == 4) {
    for (int i = 1; i <= 90; ++i) {
      if (Weapon::k4StarLevelWeaponEXPList[i] <= this->weapon_exp_) {
        level = i;
      } else {
        break;
      }
  ...
  if (rarity == 5) {
    for (int i = 1; i <= 90; ++i) {
      if (Weapon::k5StarLevelWeaponEXPList[i] <= this->weapon_exp_) {
        level = i;
      } else {
        break;
      }
...
int Weapon::GetRarity() {
  for (const auto& weapon_info : Weapon::kWeaponInfoList) {
    if (weapon_info.identifier == this->identifier_) {
      return weapon_info.rarity;
    }
  }

  // Throw exception if the weapon is invalid
  throw ExceptionNotAWeapon();
}

int Weapon::GetRefinement() { return this->refinement_; }

int Weapon::GetWeaponEXP() { return this->weapon_exp_; }

bool Weapon::CheckIsWeapon(ItemStack* item) {
  auto identifier = item->getTypeName();

  for (const auto& weapon_info : Weapon::kWeaponInfoList) {
    if (weapon_info.identifier == identifier) {
      return true;
    }
  }
...
const std::vector<Weapon::WeaponInfo> Weapon::kWeaponInfoList = {
    {"genshicraft:dull_blade", 1},
    {"genshicraft:silver_sword", 2},
    {"genshicraft:harbinger_of_dawn", 3},
    {"genshicraft:the_flute", 4},
    {"genshicraft:mistsplitter_reforged", 5}};

const std::vector<int> Weapon::k1StarLevelWeaponEXPList = {
...
const std::vector<int> Weapon::k2StarLevelWeaponEXPList = {
...
const std::vector<int> Weapon::k3StarLevelWeaponEXPList = {
...
const std::vector<int> Weapon::k4StarLevelWeaponEXPList = {
...
const std::vector<int> Weapon::k5StarLevelWeaponEXPList = {
